...
def main():
    ...
    cmd = "eve %d %d | dotnet \'/usr/local/bin/ERN_Core.dll\' \'%s/\' | getmodelERN > \'%s\'" % (event_batch, max_event_batch, rootPath, getModelOutbin)
...
def oldgulcalc():
    ...
    rutaAME="/usr/local/bin/ASLAC2_V2.4- 9int -CVMax_Parametrico.AME" #ame path
    coverageTemp= os.path.join(rootPath,'work',os.path.basename(tempfile.NamedTemporaryFile().name))
    itemsTemp= os.path.join(rootPath,'work',os.path.basename(tempfile.NamedTemporaryFile().name))
